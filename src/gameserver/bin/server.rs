...
fn handle_movement(mov: MOVEMENT, cur_coords: &mut Coordinate) -> Response  {
    ...
    Response { box_coordinate: Some(*cur_coords), write_char: (None), win_score: None }
}
...
async fn request_handler(board_state: &mut Box<BoardState>) -> Response {
    match board_state.get_action() {
        ...
        Action::WRITE(ch) => {
            ...
            Response {box_coordinate: Some(*board_state.get_current_coord()), write_char: Some(ch), win_score: Some(0)}
        }, 
        _ => {
            Response {box_coordinate: None, write_char: None, win_score: None}
        }
    }
}
